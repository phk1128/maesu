...
// 카테고리/서브파트별 공식 조회
function getFormulasBy(catId, subId) {
...
// 시험일 D-day 계산 헬퍼
function examDdayInfo(school, now = Date.now()) {
...
function getProblems(school, year) {
...
function generateContribGrid() {
  ...
  // 시작 = 오늘이 들어가는 주의 마지막 요일에서 weeks주 전 일요일
  const dow = today.getDay();
  const sundayEnd = new Date(today); sundayEnd.setDate(today.getDate() + (6 - dow));
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    for (let d = 0; d < days; d++) {
      ...
      // 패턴: 최근 ~ 활발, 주말 좀 더, 가끔 0
      let level = 0;
      if (!future) {
        const distance = Math.floor((today - date) / 86400000);
...
const CONTRIB_GRID = generateContribGrid();
const TOTAL_STUDIED = CONTRIB_GRID.flat().reduce((s, c) => s + c.level * 1.2, 0) | 0;
...
Object.assign(window, {
  FORMULA_TREE, FORMULAS, getFormulasBy, getFormula,
  SCHOOLS, PROBLEMS, getProblems, examDdayInfo,
  ANALYSIS_RESULT,
  CONTRIB_GRID, TOTAL_STUDIED, CURRENT_STREAK,
});
