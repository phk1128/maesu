// 편입 수학 - AI 기출 분석 & AI 기출 변형 (둘 다 PRO 잠금)

// ═══════════════════════════════════════════
// AI 기출 분석 페이지 (이미지 업로드)
// ═══════════════════════════════════════════
function AnalyzePage({ go, user, isPro, unlockPro, params, markStudied }) {
  ...
  // preset으로 들어왔을 때 미리 로드
  React.useEffect(() => {
    if (params?.preset) {
      const fakeProblemImage = {
        latex: params.preset.latex,
        school: params.preset.school,
        year: params.preset.year,
      };
      setImage(fakeProblemImage);
    }
  }, []);
  ...
  const onFileChange = (e) => {
    ...
    reader.onload = () => setImage({ src: reader.result, name: file.name });
...
// 로딩 뷰
// ═══════════════════════════════════════════
function LoadingView({ stages, stageIdx, image }) {
  return (
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {recommended.map((r, idx) => (
          <RecommendCard key={r.id} rec={r} idx={idx + 1} go={go}
            ...
            onFeedback={(v) => setFeedback(p => ({ ...p, [r.id]: p[r.id] === v ? null : v }))}
          />
        ))}
      </div>
...
function RecommendCard({ rec, idx, go, feedback, onFeedback }) {
  ...
  const levelTone = isEssential ? 'default' : 'muted';
...
function FeedbackBtn({ active, type, onClick }) {
  return (
...
Object.assign(window, { AnalyzePage, LoadingView, ResultView });
