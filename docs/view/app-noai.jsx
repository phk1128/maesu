...
function App() {
  ...
  const markStudied = (type, id) => { setHistory(prev => [{ type, id, ts: Date.now() }, ...prev].slice(0, 50)); };
  ...
  const signOut = () => { setUser(null); setFavorites([]); setHistory([]); go('home'); showToast('로그아웃 됐어요'); };
  ...
  const renderPage = () => {
    switch (cur.route) {
      case 'home':              return <HomePage go={go} user={user} signIn={signIn} primarySchool={primarySchool} setPrimarySchool={setPrimarySchool} hideAI />;
      ...
      case 'mypage':            return <MyPage go={go} user={user} isPro={false} favorites={favorites} history={history} signOut={signOut} />;
      default:                  return <HomePage go={go} user={user} signIn={signIn} primarySchool={primarySchool} setPrimarySchool={setPrimarySchool} hideAI />;
    }
  };
...
function MountApp() {
  ...
  React.useEffect(() => {
    const compute = () => {
      ...
      const padding = 32;
      setScale(Math.min(1, (vw - padding) / W, (vh - padding) / H));
    };
    compute();
  ...
  return (
    <div style={{
      ...
      background: '#EFEDE8', padding: 16, boxSizing: 'border-box', overflow: 'hidden',
    }}>
