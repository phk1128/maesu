...
export default function VariationsPage({ go, isPro, unlockPro, markStudied }: VariationsPageProps) {
  const [selectedSchool, setSelectedSchool] = useState<string | null>(null);
  const [phase, setPhase] = useState<'select' | 'timer' | 'result'>('select');
  ...
  useEffect(() => {
    if (phase !== 'timer' || !running || time <= 0) return;
    const t = setTimeout(() => {
      setTime(s => {
        const next = s - 1;
  ...
  const startSet = (schoolId: string) => {
    if (!isPro) { setLockDismissed(false); return; }
  ...
  if (phase === 'timer' && selectedSchool) {
    return <TimerView school={SCHOOLS.find(s => s.id === selectedSchool)!} time={time} running={running} setRunning={setRunning} problemIdx={problemIdx} setProblemIdx={setProblemIdx} finish={finish} reset={reset} />;
  }
  if (phase === 'result') {
...
// ─── Timer View ───
function TimerView({ school, time, running, setRunning, problemIdx, setProblemIdx, finish, reset }: {
  school: typeof SCHOOLS[number];
  time: number;
  running: boolean;
  setRunning: (v: boolean) => void;
  problemIdx: number;
  setProblemIdx: (v: number) => void;
  finish: () => void;
  reset: () => void;
}) {
  const variationProblems = useMemo(() => {
